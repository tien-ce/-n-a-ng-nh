...
void SubscribeRPC(CallBack &callback)
{
    ...
    { // Đăng ký nhận lệnh điều khiển từ ThingsBoard (RPC)
    ...
    Serial.println("Subcribe to RPC and Atributed Callback");
}
...
void TaskSoid(void *pvParameters)
{
    ...
    SOID_VAL *soid_val = (SOID_VAL *)pm.other;
    for (;;)
    {
        int soid = analogRead(pm.get_Pin());
        soid_val->soid = map(soid, 0, 4095, 0, 100);
        delay(Delay);
    }
}
...
void TaskPublishDataToThingsboard(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (millis() - previousDataSend > telemetrySendInterval)
        {
            ...
            int soid = thingsboard.soid_val->soid;
            ...
            // tb.sendTelemetryData("soil", soid);
            if (soid == -1)
            {
                telemetry["soil"] = NULL;
            }
            else{
                telemetry["soil"] = thingsboard.soid_val->soid;
            }
            if (thingsboard.light_val->light == -1)
            {
                telemetry["light"] = NULL;
                // tb.sendTelemetryData("light", 50);
            }
            else
            {
